...
//
    ...
    public void addTab(Context context, Class frameLayoutClass, TabParam tabParam) {
...
//        }
        ...
        holder.backagroundLayout = (LinearLayout) view.findViewById(R.id.tab_icon2);
        holder.tabTitle = ((TextView) view.findViewById(R.id.tab_title));
        if (holder.tabIndex == 2) {
            holder.badgeView = new QBadgeView(mContext).bindTarget(holder.backagroundLayout);
        }
        if (TextUtils.isEmpty(tabParam.title)) {
    ...
    public void setCheckedTab(){
        ViewHolder holder = mViewHolderList.get(0);
        showFragment(holder);
        if (mTabSelectListener != null) {
            mTabSelectListener.onTabSelected(holder);
        }
...
//        }
    ...
    private static class ViewHolder {
        public LinearLayout backagroundLayout;
