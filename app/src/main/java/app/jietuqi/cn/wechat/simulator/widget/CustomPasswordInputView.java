...
/**
 * 作者： liuyuanbo on 2019/1/26 12:20.
 * 时间： 2019/1/26 12:20
 * 邮箱： 972383753@qq.com
 * 用途： 密码 输入框
 */
public class CustomPasswordInputView extends AppCompatEditText {
     ...
     * 实心圆的半径
     */
    private int mCircleRadius = dip2px(getContext(), 7);
     ...
     * 分割线的颜色
     */
    private int mDevideLineColor = Color.GREEN;
     ...
     * 分割线的宽度
     */
    private int mDivideLineWidth = dip2px(getContext(), 1f);
    ...
    private void initPaint() {
        ...
        mStrokePaint = getPaint(dip2px(getContext(), 1f), Paint.Style.STROKE, mStrokeColor);

        mDivideLinePaint = getPaint(mDivideLineWidth, Paint.Style.FILL, mDevideLineColor);
    }
...
//        super.onDraw(canvas);
    ...
    private void drawWeChatBorder(Canvas canvas) {
        ...
        for (int i = 0; i < mMaxCount - 1; i++) {
            canvas.drawLine((i + 1) * mDivideLineWStartX, 0 + 1, (i + 1) * mDivideLineWStartX, mHeight - 1, mDivideLinePaint);
        }
