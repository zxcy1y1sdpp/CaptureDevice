...
class WechatSingleChatActivity : BaseWechatActivity() {
    ...
    private var mList: MutableList<SingleTalkEntity> = mutableListOf()
    ...
    override fun getAttribute(intent: Intent) {
        ...
        var myInfo: WechatUserEntity = UserOperateUtil.getMySelf()
...
//            var entity: AlipayScreenShotEntity = AlipayScreenShotEntity()
            var id: Int
            when(v.id){
                R.id.mSendMsgBtn ->{
                    var entity  = SingleTalkEntity(mEntity.wechatUserId, 0, mComMsg, mContentEt.text.toString(), 0)
                ...
                R.id.mSendImgIv ->{
                    var entity = if (bol){
                        SingleTalkEntity(mEntity.wechatUserId, 1, mComMsg, R.mipmap.timg, 0)
                    }else{
                        SingleTalkEntity(mEntity.wechatUserId, 1, mComMsg, R.mipmap.timg1, 0)
                    }
                    bol = !bol
                    id = mHelper.saveWechatSingleMsg(entity)
                    entity.id = id
                ...
                R.id.mSendRedPackageBtn ->{
                    var entity = SingleTalkEntity(mEntity.wechatUserId, 3, mComMsg, false, "100", mContentEt.text.toString(), 0)
                    id = mHelper.saveWechatSingleMsg(entity)
                    entity.id = id
                    mList.add(entity)
                    mAdapter?.itemCount?.minus(1)?.let { mAdapter?.notifyItemInserted(it) }
    ...
    fun onMessageEvent(entity: SingleTalkEntity) {
    ...
    fun onSelectTimeEvent(timeEntity: EventBusTimeEntity) {
        var entity = SingleTalkEntity(mEntity.wechatUserId, 2, mComMsg, timeEntity.timeLong, timeEntity.timeLong)
